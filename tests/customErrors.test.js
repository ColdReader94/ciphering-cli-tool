...
describe('Provided config check', () => {
  ...
  it('Wrong Atbash argument error check', () => {
    expect(new WrongAtbashCipher().name).toBe(
      'Atbash cipher config shouldn`t contains any other symbols after "A"'
    );
  });

  it('Wrong Caeseargument error check', () => {
    expect(new WrongCaeserCipher().name).toBe(
      'Caeser cipher config should contains number 0 or 1 after "C"'
    );
  });

  it('Wrong Rot8 argument error check', () => {
    expect(new WrongRot8Cipher().name).toBe(
      'ROT8 cipher config should contains number 0 or 1 after "R"'
    );
  });
});
